...
function boot(){installDrivePanel();shortcuts();imageFallbacks();const driveEnabled=()=>!!(window.JOGAHUB_DRIVE_SYNC?.getUrl?.()||localStorage.getItem(KEY)||DEFAULT_URL);window.addEventListener('online',function(){toast('Conexão restaurada.');if(driveEnabled()&&window.JOGAHUB_DRIVE_SYNC)window.JOGAHUB_DRIVE_SYNC.sync()});window.addEventListener('offline',function(){toast('Você está offline. Conteúdo local disponível continua acessível.')});if(driveEnabled()&&window.JOGAHUB_DRIVE_SYNC)setInterval(function(){if(navigator.onLine)window.JOGAHUB_DRIVE_SYNC.sync().then(function(n){if(n){refresh();toast('Novos conteúdos do Drive encontrados: '+n)}})},600000)}
