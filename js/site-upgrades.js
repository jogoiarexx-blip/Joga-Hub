...
function refresh(){try{window.JOGAHUB_RENDER_CURRENT?.()}catch(e){console.warn('JogaHub refresh',e)}}
function installDrivePanel(){
 ...
 wrap.innerHTML='<div style="display:flex;gap:10px;align-items:flex-start"><div style="font-size:22px">☁️</div><div style="min-width:0;flex:1"><strong style="display:block;font:800 13px system-ui;color:#fff">Acervo único do Google Drive</strong><small style="display:block;margin:5px 0 10px;color:#8997a9;line-height:1.45">Cole a URL <b>/exec</b> do Google Apps Script para importar todos os filmes, séries e subpastas do acervo principal.</small><input id="jhDriveUrl" type="url" placeholder="https://script.google.com/macros/s/.../exec" autocomplete="off" style="width:100%;box-sizing:border-box;padding:10px 11px;border-radius:9px;border:1px solid rgba(255,255,255,.1);background:#0a1018;color:#fff"><div style="display:flex;gap:7px;flex-wrap:wrap;margin-top:9px"><button id="jhDriveSave" type="button" style="padding:9px 11px;border:0;border-radius:9px;background:linear-gradient(135deg,#27c7f5,#765cff);color:#fff;font:800 11px system-ui;cursor:pointer">Sincronizar agora</button><button id="jhDriveClear" type="button" style="padding:9px 11px;border:1px solid rgba(255,255,255,.1);border-radius:9px;background:#141c28;color:#c8d3df;font:700 11px system-ui;cursor:pointer">Atualizar do zero</button></div><div id="jhDriveStatus" style="margin-top:8px;color:#718095;font:600 10px system-ui"></div></div></div>';
 panel.appendChild(wrap);
 const input=wrap.querySelector('#jhDriveUrl'),status=wrap.querySelector('#jhDriveStatus');input.value=localStorage.getItem(KEY)||DEFAULT_URL||'';
 function paint(){const custom=localStorage.getItem(KEY);status.textContent=input.value?(custom?'✓ URL personalizada configurada — sincronização automática habilitada.':'✓ Google Drive conectado automaticamente ao Apps Script publicado.'):'Cole a URL /exec para habilitar a sincronização.'}paint();
...
function boot(){installDrivePanel();shortcuts();imageFallbacks();window.addEventListener('online',function(){toast('Conexão restaurada.')});window.addEventListener('offline',function(){toast('Você está offline. Conteúdo local disponível continua acessível.')});setInterval(function(){if(document.visibilityState==='visible'&&navigator.onLine&&window.JOGAHUB_DRIVE_SYNC?.getUrl?.()&&!window.JOGAHUB_DRIVE_SYNC.getStatus().cacheFresh)window.JOGAHUB_DRIVE_SYNC.sync().then(function(n){if(n)toast('Novos conteúdos do Drive encontrados: '+n)})},600000)}
