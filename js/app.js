...
function registerRuntimeItems(additions){
  const seenIds=new Set([...ITEMS,...RUNTIME_ITEMS].map(item=>item.id));
  const seenMedia=new Set([...ITEMS,...RUNTIME_ITEMS].map(item=>
    item.archiveId?'archive:'+item.archiveId:item.youtubeId?'youtube:'+item.youtubeId:'').filter(Boolean));
  let changed=false;
  for(const item of additions||[]){
    const mediaKey=item?.archiveId?'archive:'+item.archiveId:item?.youtubeId?'youtube:'+item.youtubeId:'';
    if(!item?.id||seenIds.has(item.id)||(mediaKey&&seenMedia.has(mediaKey)))continue;
    RUNTIME_ITEMS.push(item);seenIds.add(item.id);
    if(mediaKey)seenMedia.add(mediaKey);
    changed=true;
  }
  if(changed)scheduleCatalogRefresh();
}
...
const CURRENT_SHELL_CACHE = 'jogahub-1.3.4';
...
function itemHref(item){
  ...
  if(item.type === 'filme' && item.driveFolderId){
    return item.sourceUrl || `https://drive.google.com/drive/folders/${encodeURIComponent(item.driveFolderId)}`;
  }
...
function itemLinkAttrs(item){
  ...
  if(item.type === 'filme' && item.driveFolderId) return ' target="_blank" rel="noopener noreferrer"';
...
function seriesCardHTML(group){
  ...
  const episodes=group.items.some(i=>Number(i.seasonEpisodeCount)>0) ? group.items.reduce((sum,i)=>sum+(Number(i.seasonEpisodeCount)||0),0) : (Number(first.episodeCount||0) || (catalogOnly ? Number(first.episodeCount||0) : group.items.length));
...
function applyFilters(){
  ...
  if(search) search.placeholder = activeType === 'todos' ? 'buscar jogos, filmes, séries e animes...' : activeType === 'filme'
    ...
    : activeType === 'emulador' ? 'buscar emulador...' : 'buscar jogo por título, gênero ou descrição...';
  const empty = document.getElementById('emptyState');
  if(empty) empty.textContent = activeType === 'filme' ? 'nenhum filme encontrado com esse filtro.' : activeType === 'serie' ? 'nenhuma série encontrada com esse filtro.' : activeType === 'anime' ? 'nenhum anime encontrado com esse filtro.' : activeType === 'emulador' ? 'nenhum emulador encontrado.' : 'nenhum jogo encontrado com esse filtro.';
}
...
async function loadArchivePicaPau(){
  ...
  try{
    ...
    if(additions.length) registerRuntimeItems(additions);
...
function loadMediaDiscoveriesOnce(){
  if(mediaDiscoveryStarted||!['filme','serie','anime'].includes(activeType)||!navigator.onLine)return;
  mediaDiscoveryStarted=true;
  // A home carrega primeiro; pesquisas externas começam apenas na área de mídia.
  setTimeout(()=>{
...
window.addEventListener('online',loadMediaDiscoveriesOnce);
